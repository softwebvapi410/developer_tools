...
header('Content-Type: application/json');
        ...
        if ($html && strpos($info['content_type'] ?? '', 'text/html') !== false) {
            $dom = new DOMDocument();
            ...
            if (isset($parsedUrl['scheme']) && $parsedUrl['scheme'] === 'https') {
                ...
                $seo['issues'][] = "Site is not using HTTPS — SSL certificate missing or not enforced.";
                $seo['score'] -= 20;
            }
            ...
            // ===== META KEYWORDS (for keyword density seed) =====
            ...
            // Detect schema types by string matching too
            ...
            // ===== KEYWORD DENSITY (title → description → body fallback) =====
            $metaKwList = [];
            ...
            // Build keyword source map: title → description → body (priority cascade)
            $titleWords = [];
            if (!empty($seo['title'])) {
            ...
            // Boost: title keywords get highest boost, then description, then meta keywords
            $ranked = $freq;
            foreach ($metaKwList as $mk => $_) {
                if (isset($ranked[$mk])) $ranked[$mk] *= 2;
            }
            foreach ($descWords as $dk => $_) {
                if (isset($ranked[$dk])) $ranked[$dk] *= 3;
            }
            foreach ($titleWords as $tk => $_) {
                if (isset($ranked[$tk])) $ranked[$tk] *= 5;
            }
            arsort($ranked);
            ...
            foreach ($topKw as $kw => $boostedCount) {
                ...
                // Determine source: title > description > meta > body
                $source = 'body';
            ...
            $chR = curl_init();
            curl_setopt_array($chR, [CURLOPT_URL=>$robotsUrl, CURLOPT_RETURNTRANSFER=>true, CURLOPT_FOLLOWLOCATION=>true, CURLOPT_SSL_VERIFYPEER=>false, CURLOPT_TIMEOUT=>5]);
            $rc = curl_exec($chR);
            ...
            // Extract Sitemap: refs from robots.txt
            ...
            foreach ($sitemapCandidates as $smCandidate) {
                $chSm = curl_init();
                curl_setopt_array($chSm, [CURLOPT_URL=>$smCandidate, CURLOPT_NOBODY=>true, CURLOPT_RETURNTRANSFER=>true, CURLOPT_SSL_VERIFYPEER=>false, CURLOPT_TIMEOUT=>5, CURLOPT_FOLLOWLOCATION=>true]);
                curl_exec($chSm);
            ...
            $chLlms = curl_init();
            curl_setopt_array($chLlms, [CURLOPT_URL=>$llmsUrl, CURLOPT_NOBODY=>true, CURLOPT_RETURNTRANSFER=>true, CURLOPT_SSL_VERIFYPEER=>false, CURLOPT_TIMEOUT=>5, CURLOPT_FOLLOWLOCATION=>true]);
            curl_exec($chLlms);
            ...
            // Uses specific regex patterns to avoid false positives from short/generic strings.
            // Each pattern is anchored to a real, unique script URL, function call, or identifier.
            $trackingDefs = [
                // ---- Analytics ----
                [
                    ...
                    'regex'=>[
                        ...
                        '/gtag\s*\(\s*[\'"]config[\'"]\s*,\s*[\'"]G-[A-Z0-9]{4,}[\'"]/i',
                    ],
                ],
                [
                    ...
                    'regex'=>[
                        ...
                        '/ga\s*\(\s*[\'"]create[\'"]\s*,\s*[\'"]UA-[0-9]+-[0-9]+[\'"]/i',
                        '/_gaq\.push\s*\(\s*\[\s*[\'"]_setAccount[\'"]/i',
                    ],
                ],
                [
                    ...
                    'regex'=>[
                        ...
                        '/\(function\s*\(w,d,s,l,i\)\s*\{w\[l\]=w\[l\]\|\|\[\]/i',
                    ],
                ],
                [
                    ...
                    'regex'=>[
                        ...
                        '/analytics\.load\s*\(\s*[\'"][a-zA-Z0-9]{10,}[\'"]/i',
                    ],
                ],
                [
                ...
                [
                    ...
                    'regex'=>[
                        ...
                        '/gtag\s*\(\s*[\'"]config[\'"]\s*,\s*[\'"]AW-[0-9]{7,}[\'"]/i',
                ...
                [
                    ...
                    'regex'=>[
                        ...
                        '/_linkedin_partner_id\s*=\s*[\'"]?\d{4,}/i',
                    ],
                ],
                [
                    ...
                    'regex'=>[
                        ...
                        '/twq\s*\(\s*[\'"]init[\'"]\s*,\s*[\'"][a-z0-9]{5,}[\'"]/i',
                    ],
                ],
                [
                    'id'=>'pinterest', 'name'=>'Pinterest Tag', 'category'=>'Advertising', 'color'=>'#e60023', 'icon'=>'image',
                    'regex'=>[
                        '/ct\.pinterest\.com\/v3\/\?tid=/i',
                        '/pintrk\s*\(\s*[\'"]load[\'"]\s*,\s*[\'"]\d{10,}[\'"]/i',
                ...
                [
                    ...
                    'regex'=>[
                        ...
                        '/snaptr\s*\(\s*[\'"]init[\'"]\s*,\s*[\'"][a-f0-9\-]{30,}[\'"]/i',
                        '/tr\.snapchat\.com\/p\.svg\?id=/i',
                    ],
                ],
                // ---- Heatmap / UX ----
                [
                    ...
                    'regex'=>[
                        '/static\.hotjar\.com\/c\/hotjar-\d+\.js/i',
                        '/hjid\s*:\s*\d+\s*,\s*hjsv\s*:/i',
                    ],
                ],
                [
                    'id'=>'clarity', 'name'=>'Microsoft Clarity', 'category'=>'Heatmap / UX', 'color'=>'#0067b8', 'icon'=>'eye',
                    'regex'=>[
                        '/www\.clarity\.ms\/tag\/[a-z0-9]+/i',
                        '/clarity\s*\(\s*[\'"]set[\'"]/i',
                    ],
                ],
                // ---- Chat / Support ----
                [
                    ...
                    'regex'=>[
                        '/widget\.intercom\.io\/widget\/[a-z0-9]+/i',
                        '/intercomSettings\s*=\s*\{/i',
                        '/Intercom\s*\(\s*[\'"]boot[\'"]/i',
                    ],
                ],
                [
                    ...
                    'regex'=>[
                        ...
                        '/CRISP_WEBSITE_ID\s*=\s*[\'"][a-f0-9\-]{30,}[\'"]/i',
                    ],
                ],
                [
                ...
                [
                    ...
                    'regex'=>[
                        '/js\.hs-scripts\.com\/\d{5,}\.js/i',
                        '/js\.hsforms\.net\/forms\/v2\.js/i',
                        '/_hsq\s*=\s*window\._hsq\s*\|\|/i',
                    ],
                ],
            ];
...
<div style='font-family: Arial, sans-serif; padding: 16px 20px; background:#fff; border: 1px solid #dfe1e5; border-radius: 8px; line-height:1.4;'>
    ...
    <div style='color:#4d5156; font-size:14px;'>{$serpDesc}</div>
            ...
            $seo['people_also_ask'] = [
                ...
                "Benefits of {$ts}?",
                "Is {$ts} worth it?",
                "How much does {$ts} cost?",
            ];

            // ===== RELATED SEARCHES (simulated) =====
            ...
            foreach ($dom->getElementsByTagName('a') as $node) {
                ...
                // Strip fragment and query, then normalize scheme+host to lowercase, preserve path case
                $href = strtok(strtok($href, '#'), '?');
                $href = rtrim($href, '/');
                ...
                $cleanLower = strtolower($clean); // for domain-matching only
                if ($baseDomain && strpos($cleanLower, strtolower($baseDomain)) !== false && strpos($cleanLower, strtolower($baseUrl)) === 0) {
                    if (!preg_match('/\.(jpg|jpeg|png|gif|webp|zip|css|js|pdf|svg|ico|woff|woff2)$/i', $clean)) {
                        $internalLinks[] = $clean;
                        $links[] = $clean;
                    }
            ...
            // ===== BROKEN LINKS (sample up to 15 external) =====
            $checkLinks = array_slice(array_unique($externalLinks), 0, 15);
            $mh = curl_multi_init();
            $handles = [];
            foreach ($checkLinks as $i => $cl) {
                $ch2 = curl_init();
                curl_setopt_array($ch2, [CURLOPT_URL=>$cl, CURLOPT_NOBODY=>true, CURLOPT_RETURNTRANSFER=>true, CURLOPT_TIMEOUT=>5, CURLOPT_SSL_VERIFYPEER=>false, CURLOPT_FOLLOWLOCATION=>true, CURLOPT_MAXREDIRS=>2]);
                $handles[$i] = $ch2;
